...
# Brotli first: nginx links -lbrotlienc out of deps/brotli/out, so
# the library has to exist before nginx is built. Static, to keep
# the test runs free of LD_LIBRARY_PATH handling. The "brotli"
# target pulls in the libraries and adds the command line tool,
# which the shell suite decompresses responses with.
