...
/* Brotli and GZip modules never stack, i.e. when one of them sets
   ...
   IIUC, buffered == some data passed to filter has not been pushed
   further. */
#define NGX_HTTP_BROTLI_BUFFERED NGX_HTTP_GZIP_BUFFERED

/* How much input may be held back while waiting to learn the response
   size. Chosen to match Brotli's input block size at the qualities
   that use one: the encoder would have buffered that much internally
   without emitting anything, so holding it here is not observable.
   Note the block is only 64k for quality 4 and above - below that
   Brotli uses 16k (see ComputeLgBlock), and this cap is larger than
   the encoder's own. That costs nothing today because the deferral
   almost always ends earlier, when the caller asks for progress with
   a NULL chain, but it is why this is a ceiling, not a target. */
...
/* What, if anything, the single output buffer is currently holding.
   ...
   from ready to busy. They were once two independent bits, which left
   a fourth combination that could not arise and had to be reasoned
   away on every read. */
typedef enum {
...
/* What one turn of the body filter's loop decided to do next. The
   loop owns the returns; a step only says which one, so that each
   phase can be read on its own without tracing control flow back out
   through the loop. */
typedef enum {
...
/* What the body filter should do once ngx_http_brotli_filter_prepare
   has settled the decisions that come before the encoder. This is
   deliberately not an ngx_int_t: the alternative was a sentinel
   return code, which would have had to be a value no filter below
   could ever hand back, and nothing but a comment would have kept
   that true.
   ...
   the control flow. "Not yet" and "no" answer different questions,
   and the response may still be compressed after a DEFER. */
typedef enum {
    /* Accepted for encoding: nothing left to settle, so carry on into
...
typedef struct {
    ...
    /* 1 once a buffer marked last_buf has been handed to the encoder.
       Never cleared: input cannot resume. Read to choose FINISH over
       FLUSH when draining, and to know the stream may be closed once
       the encoder says it is finished. */
    unsigned end_of_input : 1;
    /* 1 when a block has been closed and the next buffer taken from
       the encoder must carry the flush marker, so the filters below
       push it out instead of holding it for more. Despite sitting
       beside end_of_input this is an instruction rather than a state:
       take_output clears it as soon as it has acted on it. */
    unsigned flush_pending : 1;
    /* 1 if input has been handed to the encoder that it has not been
       asked to emit yet. BROTLI_OPERATION_PROCESS holds such data
       back until a block fills, so it has to be flushed explicitly
       when the caller wants output. */
    ...
    /* 1 if this call of the body filter arrived with no new data,
       ...
       over rather than adding to it. Set once per call, before the
       loop runs. */
...
static ngx_conf_num_bounds_t ngx_http_brotli_comp_level_bounds = {
    ngx_conf_check_num_bounds, BROTLI_MIN_QUALITY,
    BROTLI_MAX_QUALITY};
...
ngx_http_brotli_header_filter(ngx_http_request_t *r)
{
    ...
    /* Bypass statuses that either carry no body, or carry one that
       ...
       This is deliberately a deny list. The allow list it replaces
       (200/403/404 only, dropped in eebeaf3) also excluded perfectly
       compressible responses such as 201, 422 and 500; those stay
       compressed. */
    ...
    /* When the length is unknown there is nothing yet to compare
       against brotli_min_length, and committing the headers here
       ...
       decide, which takes only brotli_min_length bytes rather than
       the whole response. */
...
ngx_http_brotli_filter_send_headers(ngx_http_brotli_ctx_t *ctx)
{
    ...
    if (!ctx->accepted_for_compression) {
        /* Nothing has been allocated yet on this path - headers are
           ...
           it the only thing that sets ctx->closed, so the flag cannot
           mean two things. */
...
/* Hands the committed output buffer to the next filter, and reports
   ...
   while any of its output is still outstanding: out_buf points into
   memory Brotli owns.

   The loop calls this only while ctx->output is not IDLE, so the
   buffer is always in one of two states on entry, and "resend" says
   which. */
...
ngx_http_brotli_filter_send_output(ngx_http_brotli_ctx_t *ctx)
{
    ...
    if (rc == NGX_OK) {
        /* A resend that moved nothing means the filters below are
           holding the buffer and will not take more of it now, so
           going round again would only ask a second time. A first
           send is never a stall, however little of it was taken. */
    ...
    if (rc == NGX_AGAIN) {
        if (ctx->output == NGX_HTTP_BROTLI_OUTPUT_BUSY) {
            /* Cannot compress on; let the outer filter decide. */
            if (ctx->in != NULL) {
                r->connection->buffered |= NGX_HTTP_BROTLI_BUFFERED;
            }
            return NGX_HTTP_BROTLI_STEP_AGAIN;
...
/* Wraps whatever the encoder has produced in out_buf, and marks it
   last or flush if this block ends the stream or a meta-block. */
static ngx_http_brotli_step_e
...
/* Runs the encoder with no new input, which is how it is made to
   release what it is already holding: FINISH once the input has
   ended, FLUSH before then. Which one to use follows from
   ctx->end_of_input rather than from an argument, because those are
   the only terms on which either is correct. Both leave output for
   the next turn of the loop to collect, which is why both mark the
   connection buffered. */
static ngx_http_brotli_step_e
...
/* Advances the encoder now that it holds no output: finishes the
   stream, drains what a flush would release, or pushes the next input
   buffer in. */
static ngx_http_brotli_step_e
ngx_http_brotli_filter_feed_encoder(ngx_http_brotli_ctx_t *ctx)
{
    ...
    if (ctx->in == NULL) {
        /* Nothing left to compress. If the caller is asking for
           output and the encoder is holding input that PROCESS will
           not emit until a block fills, ask for it now: without this
           a slowly-produced response stalls until 64 KB has
           accumulated. Costs a few percent of ratio, because each
           flush closes a meta-block early. */
        if (ctx->caller_wants_output && ctx->unflushed_input) {
            ctx->unflushed_input = 0;
            /* Mark the resulting buffer so the filters below push it
               out rather than holding it for more. */
            ctx->flush_pending = 1;
            return ngx_http_brotli_filter_drain_encoder(ctx);
        }
        return NGX_HTTP_BROTLI_STEP_DONE;
...
static size_t
ngx_http_brotli_filter_pending_input(ngx_chain_t *in,
    ...
    ngx_int_t                                        *rc)
{
    ...
    pending = ngx_http_brotli_filter_pending_input(ctx->in, &complete,
        &urgent);

    /* Headers held back because the length was unknown. Decide as
       soon as the body answers the only question brotli_min_length
       asks - is it at least that big - which needs min_length bytes,
       not the whole response, so this costs no meaningful latency. A
       flush marker means something downstream is waiting, so decide
...
/* Initializes encoder, output chain and buffer, if necessary. Returns
   NGX_OK if encoder is successfully initialized (have been already
    ...
    ngx_http_brotli_ctx_t *ctx)
{
    ...
    if (ctx->content_length > 0) {
        ...
        while ((wbits < conf->lg_win) &&
               (ctx->content_length > (1 << wbits))) {
            wbits++;
        }
    } else {
        wbits = conf->lg_win;
    ...
    /* Encoder memory is not owned by the pool, so arrange for it to
       ...
       can not strand an allocated instance. */
    ...
    ctx->encoder =
        BrotliEncoderCreateInstance(ngx_http_brotli_filter_alloc,
    ...
    /* Last, so that the flag means what it says: the encoder exists
       and both output objects are allocated. Every failure above
       returns NGX_ERROR with it still clear, and the caller closes
       the stream on that, so the half-built state is never seen again
       - ctx->closed makes every later call a straight hand-off. */
    ctx->initialized = 1;
...
ngx_http_brotli_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    /* Main loop, one phase per turn:
       - output still outstanding - push it down, and do not touch the
       encoder until it has all been consumed
       - encoder has output - wrap it
       - otherwise - advance the encoder: finish, flush, or feed it
       input

       Each phase returns what to do next rather than returning from
       here itself, so the four ways out of this filter are all
       written once, in the switch below. */
    for (;;) {
...
/* Releases the encoder if the request is terminated before
   compression is finished, i.e. when ngx_http_brotli_filter_close is
   never reached. */
static void
ngx_http_brotli_filter_cleanup(void *data)
{
    ...
    /* Normally the encoder is already gone:
       ngx_http_brotli_filter_close resets the field. This is the
       abort path. */
    if (ctx->encoder != NULL) {
...
ngx_http_brotli_filter_close(ngx_http_brotli_ctx_t *ctx)
{
    ...
    /* out_buf is pool memory and small, so there is nothing to hand
       back - ngx_pfree only reclaims blocks large enough to have been
       malloc'd on their own, and would just walk pool->large to say
       so. Dropping the pointer is the whole of the cleanup; the
       buffer dies with the pool. */
    ctx->out_buf = NULL;
}
...
ngx_http_brotli_merge_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    /* 4 rather than the 6 this module used to default to. Measured
       against script/corpus with a release build: the corpus
       compresses to 228,062 bytes at quality 4 against 212,327 at 6
       - 7.4% larger - for 10.0 ms of encoder time against 18.6, a
       little over half. Unlike the lg_win change below this buys
       nothing in memory: peak live encoder bytes are flat across the
       range, 1.94 MB at 4 against 1.93 MB at 6. It trades ratio for
       CPU, and only that.

       Worth knowing before moving it again: 4 is not the knee of the
       curve. Quality 5 costs 0.8% in ratio against 6 and saves 20% of
       the time, where the further step down to 4 costs another 6.6%
       in ratio to save another 33%. Most of the CPU is available at 5
       for almost none of the bytes; going to 4 is a deliberate choice
       to spend bytes on latency. */
    ngx_conf_merge_value(conf->quality, prev->quality, 4);
    /* 16 bits (64k) rather than the 19 (512k) this module used to
       default to. Brotli picks a much cheaper hasher at lg_win <= 16,
       so 64k is the largest window before encoder memory jumps:
       measured against script/corpus, a streamed response peaks near
       0.99 MB here against 4.0 MB at 512k. What that costs in ratio
       depends on the content, since it only bites where a match would
       have reached back beyond 64 KB - +0.04% on CSS, +0.4% on JS,
       +1.0% on minified JS, +2.2% on prose, +2.7% on HTML, +1.6% over
       the corpus as a whole. Those are at quality 6, which was the
       default when they were taken; at the current default of 4 the
       same change costs +1.1% over the corpus and +2.4% at its worst,
       a lower quality having fewer long-range matches to give up. 32k
       and 16k cost the same memory as 64k while compressing worse, so
       64k is the useful floor rather than the smallest possible
       value. */
    ngx_conf_merge_size_value(conf->lg_win, prev->lg_win, 16);
    /* 20 is the gzip module's default, but Brotli is a far worse deal
       on tiny responses: an encoder instance costs ~560 KB regardless
       of how little it is asked to compress, and measured against
       realistic JSON the compressed body only starts beating the
       original around 96 bytes - closer to 128 once the
       "Content-Encoding" header is paid for. 256 clears that with
       room to spare. */
